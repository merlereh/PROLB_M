...
public:
  ...
  : Node("kf_node")
  {
      ...
      [this](nav_msgs::msg::Odometry::UniquePtr msg) -> void {
        ...
        if (!initialized_) {
          ...
          RCLCPP_INFO(this->get_logger(),
            "KF initialized: x=%.3f, y=%.3f, theta=%.3f", x, y, theta);
          return;
        }
        ...
        RCLCPP_DEBUG(this->get_logger(),
          "KF estimate: x=%.3f, y=%.3f, theta=%.3f, vx=%.3f, vy=%.3f, dtheta=%.3f",
    ...
    RCLCPP_INFO(this->get_logger(), "Kalman filter node started (6D state).");
...
private:
    ...
    const std::string & frame_id)
  {
    ...
    // Fill 6x6 ROS covariance matrix from our 6x6 Sigma
    for (int i = 0; i < 36; ++i) {
